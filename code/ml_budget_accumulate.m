...
for T=T_beg:T_frq:T_end
    ...
    if IF_DIAGNOSTICS
        for TT=T_beg:T_frq:T
            ...
            if TT==T_beg;
                temp_init_ml =load_data(fileIN,'temp_ml');
                ...
                if IF_SALINITY
                salt_init_ml =load_data(fileIN,'salt_ml');
                ...
                temp_rate_ml =temp_rate_ml +load_data(fileIN,'temp_rate_ml')*DT;
                temp_entr_ml =temp_entr_ml +load_data(fileIN,'temp_entr_ml')*DT;
                temp_hadv_ml =temp_hadv_ml +load_data(fileIN,'temp_hadv_ml')*DT;
                temp_vadv_ml =temp_vadv_ml +load_data(fileIN,'temp_vadv_ml')*DT;
                temp_hdiff_ml=temp_hdiff_ml+load_data(fileIN,'temp_hdiff_ml')*DT;
                temp_vdiff_ml=temp_vdiff_ml+load_data(fileIN,'temp_vdiff_ml')*DT;
                temp_nudge_ml=temp_nudge_ml+load_data(fileIN,'temp_nudge_ml')*DT;
                if HCM_DIA_OUTPUT
                if IF_HCM_DIA_RHR
                temp_rhr_ml=temp_rhr_ml+load_data(fileIN,'temp_rhr_ml')*DT;
                temp_phr_ml=temp_phr_ml+load_data(fileIN,'temp_phr_ml')*DT;
                end
                if IF_HCM_DIA_SHR
                temp_shr_ml=temp_shr_ml+load_data(fileIN,'temp_shr_ml')*DT;
                end
                end
                if IF_SALINITY
                salt_tend_ml =salt_tend_ml +load_data(fileIN,'salt_tend_ml')*DT;
                salt_rate_ml =salt_rate_ml +load_data(fileIN,'salt_rate_ml')*DT;
                salt_entr_ml =salt_entr_ml +load_data(fileIN,'salt_entr_ml')*DT;
                salt_hadv_ml =salt_hadv_ml +load_data(fileIN,'salt_hadv_ml')*DT;
                salt_vadv_ml =salt_vadv_ml +load_data(fileIN,'salt_vadv_ml')*DT;
                salt_hdiff_ml=salt_hdiff_ml+load_data(fileIN,'salt_hdiff_ml')*DT;
                salt_vdiff_ml=salt_vdiff_ml+load_data(fileIN,'salt_vdiff_ml')*DT;
                salt_nudge_ml=salt_nudge_ml+load_data(fileIN,'salt_nudge_ml')*DT;
                end
            end
        end
    end

    save(fileS,'Time','lon','lat','mask','h','zeta','MLD','*_ml')
    clear Time lon lat mask h zeta MLD *_ml
